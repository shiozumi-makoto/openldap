...
/**
 * 既存OCを保持しつつ必要OCを追加するターゲットOCを作る
 */
function build_target_objectclasses(array $existingOCs, bool $wantPosixShadow = true, bool $wantSamba = false): array {
    $required = ['top','person','organizationalPerson','inetOrgPerson'];
    $target = array_map('strval', $existingOCs);
    $target = array_merge($target, $required);
    if ($wantPosixShadow) {
        $target[] = 'posixAccount';
        $target[] = 'shadowAccount';
    }
    if ($wantSamba) {
        $target[] = 'sambaSamAccount';
    }
    $target = array_values(array_unique($target));
    return $target;
}

// ===== LDAP upsert（OC安全マージ + フォールバック）=====
        ...
        if ($entries && $entries['count'] > 0) {
            ...
            if (isset($entries[0]['objectclass'])) {
                for ($i = 0; $i < $entries[0]['objectclass']['count']; $i++) {
                    $existingOCs[] = $entries[0]['objectclass'][$i];
                }
            }
        }
    }

    // Samba の付与は既存を壊したくないのでデフォルト false（別スクリプトで管理）。
    // 必要なら true に変更可。
    $needSamba = in_array('sambaSamAccount', $existingOCs, true);

    // rfc2307bis想定（posixAccount AUX）。環境により衝突するなら後段で除外して再試行。
    $targetOCs = build_target_objectclasses($existingOCs, true, $needSamba);

    // 変更セット（属性ごとに）
    $attrsCommon = [
        ['attrib'=>'objectClass',   'values'=>$targetOCs],
        ['attrib'=>'cn',            'values'=>[(string)($u['cn'] ?? $uid)]],
        ['attrib'=>'sn',            'values'=>[(string)($u['sn'] ?? $uid)]],
        ['attrib'=>'givenName',     'values'=>[(string)($u['givenName'] ?? ($u['given_name'] ?? $uid))]],
        ['attrib'=>'displayName',   'values'=>[(string)($u['cn'] ?? $uid)]],
        ['attrib'=>'mail',          'values'=>[(string)($u['mail'] ?? '')]],
        ['attrib'=>'uidNumber',     'values'=>[(string)$u['uidNumber']]],
        ['attrib'=>'gidNumber',     'values'=>[(string)$u['gidNumber']]],
        ['attrib'=>'homeDirectory', 'values'=>[(string)$u['homeDirectory']]],
        ['attrib'=>'loginShell',    'values'=>[(string)($u['loginShell'] ?? '/bin/bash')]],
    ];
    $mods = [];
    foreach ($attrsCommon as $a) {
        $mods[] = ['modtype'=>LDAP_MODIFY_BATCH_REPLACE] + $a;
    }
    if (!empty($u['userPassword'])) {
        $mods[] = ['modtype'=>LDAP_MODIFY_BATCH_REPLACE, 'attrib'=>'userPassword', 'values'=>[(string)$u['userPassword']]];
    }

    // 既存に sambaSamAccount がある場合のみ、Samba属性も維持・更新（必要最小限）
    if ($needSamba) {
        // 値を与えない属性は変更されません
        foreach (['sambaSID','sambaNTPassword','sambaPwdLastSet','sambaPrimaryGroupSID','sambaAcctFlags'] as $sattr) {
            if (!empty($u[$sattr])) {
                $mods[] = ['modtype'=>LDAP_MODIFY_BATCH_REPLACE, 'attrib'=>$sattr, 'values'=>[(string)$u[$sattr]]];
            }
        }
    }

    // 実行関数（batch が無いPHPでも動くようにフォールバック）
    $applyMods = function($conn, $dn, $mods, $logfile) {
        if (function_exists('ldap_modify_batch')) {
            return @ldap_modify_batch($conn, $dn, $mods);
        } else {
            // 最低限のフォールバック：objectClass を含む配列を一括 replace
            $aggregate = [];
            foreach ($mods as $m) {
                $aggregate[$m['attrib']] = $m['values'];
            }
            return @ldap_mod_replace($conn, $dn, $aggregate);
        }
    };

    if ($exists) {
        if ($dry) { log_line($logfile, "[LDAP][DRY][MOD] {$dn}"); return; }
        $ok = $applyMods($conn, $dn, $mods, $logfile);
        if (!$ok) {
            // posix/shadow を外して再試行（STRUCTURAL 衝突対策）
            $reducedOCs = array_values(array_diff($targetOCs, ['posixAccount','shadowAccount']));
            foreach ($mods as &$m) {
                if ($m['attrib'] === 'objectClass') $m['values'] = $reducedOCs;
            }
            $ok2 = $applyMods($conn, $dn, $mods, $logfile);
            if ($ok2) log_line($logfile, "[LDAP][OK  ][MOD] {$dn} (without posix/shadow)");
            else      log_line($logfile, "[LDAP][ERROR] modify {$dn}: ".ldap_error($conn));
        } else {
            log_line($logfile, "[LDAP][OK  ][MOD] {$dn}");
        }
    } else {
        if ($dry) {
            log_line($logfile, "[LDAP][DRY][ADD] {$dn}");
            return;
        }
        // 新規は add で（batch は使えないので配列化）
        $entry = ['uid' => $uid];
        foreach ($mods as $m) {
            $entry[$m['attrib']] = $m['values'];
        }
        $ok = @ldap_add($conn, $dn, $entry);
        if (!$ok) {
            // 新規でも posix/shadow を外して再試行
            if (isset($entry['objectClass'])) {
                $entry['objectClass'] = array_values(array_diff((array)$entry['objectClass'], ['posixAccount','shadowAccount']));
            }
            $ok2 = @ldap_add($conn, $dn, $entry);
            if ($ok2) log_line($logfile, "[LDAP][OK  ][ADD] {$dn} (without posix/shadow)");
            else      log_line($logfile, "[LDAP][ERROR] add {$dn}: ".ldap_error($conn));
        } else {
            log_line($logfile, "[LDAP][OK  ][ADD] {$dn}");
        }
    }
}
...
foreach ($rows as $row) {
    ...
    if ($LDAP_ENABLED && $uidNumber > 0 && $gidNumber > 0) {
        ...
        $usersForLdap[] = [
            ...
            // Samba 属性を更新したい場合は以下のキーを渡せばOK（渡さない属性は触らない）
            // 'sambaSID' => 'S-1-5-21-...-<uidNumber>',
            // 'sambaNTPassword' => '<NTLM>',
            // 'sambaPwdLastSet' => '<epoch>',
            // 'sambaPrimaryGroupSID' => 'S-1-5-21-...-<gidNumber>',
            // 'sambaAcctFlags' => '[U          ]',
        ];
    } elseif ($LDAP_ENABLED) {
