 ...
 *  - 既存 objectClass を尊重し必要分のみ安全に追加/更新（posix/shadow 衝突時は段階的フォールバック）
...
// -----------------------------------------------------------------------------
// ログ出力（色つき対応 / chr(27) 生成で 0x1B 混入を回避）
// -----------------------------------------------------------------------------
...
/**
 * pad_mb
 *  - マルチバイト対応の左寄せパディング（全角=幅2として mb_strwidth で整列）
 *
 * @param string $s     表示文字列（無着色）
 * @param int    $width 表示幅
 * @return string       右側に半角スペースで詰めて width に揃える
 */
...
/**
 * colorize_by_grp
 *  - employeeType（例: "adm-cls 1"）の色付け
 *    * ent-cls 20 は無着色
 *    * err-cls は赤、tmp-cls はグレー
 *    * それ以外: adm=黄, dir=マゼンタ, mgr=青, mgs=シアン, stf=緑, ent(≠20)=緑
 *
 * @param string   $grp
 * @param string   $label   無着色テキスト
 * @param int|null $levelId
 * @return string  色付きテキスト（端末がカラー非対応なら無色）
 */
...
/**
 * format_etype
 *  - 無着色テキストを先に固定幅に整形 → 最後に色付け
 *    ※ これにより printf("%s") で渡しても列ズレしない
 *
 * @param string $grp
 * @param int    $levelId
 * @param int    $width     固定幅
 * @return string           色付きの固定幅ラベル
 */
...
Options:
  ...
  --ldap              LDAP 反映を有効化
  --ldapi             LDAP_URL が未指定時、ldapi を既定（EXTERNAL bind）
  ...
  --log=FILE          ログファイル名（将来用）
  --min-local-uid=N   予備（未使用）

Notes:
  * 住所/連絡先は投入前に正規化（全角→半角など）。Invalid syntax 時は連絡先ブロック除去で再試行。
  * list 表示・実行ログとも employeeType を色分け（ent-cls 20 は無色、err-cls=赤、tmp-cls=灰、他は階層色）。
 ...
 *  - 住所部品から postalAddress を生成（inetOrgPerson 慣習の「$」区切り）
 *
 * @param ?string $country 国または地域
 * @param ?string $pref    都道府県
 * @param ?string $city    市区町村
 * @param ?string $addr    番地
 * @param ?string $zip     郵便番号（別属性 postalCode を推奨）
 * @return ?string         例: "東京都千代田区1-2-3 $ Japan" / 全て空なら null
 */
...
/**
 * kanaToRomaji
 *  - kakasi で仮名→ローマ字（半角・小文字化、スペース/アポストロフィ除去）
 */
...
if ($LIST_ONLY) {
    foreach ($rows as $r) {
        ...
        // employeeType を整形（0 は 99 に置換）
        $levelId = (int)($r['level_id'] ?? 0);
        if ($levelId === 0) $levelId = 99;
        // $grp = GroupDef::classify($levelId) ?? 'err-cls';
		$g   = GroupDef::classify(null, null, $levelId);
		$grp = is_array($g) && isset($g['name']) ? strtolower($g['name']) : 'err-cls';

        $etype_disp = format_etype($grp, $levelId, 12); // 固定幅→色付け
...
foreach ($rows as $idx=>$row) {
    ...
    // ------------------------------------------------------------------------- ★ここから
    // LDAP upsert（ログの employeeType は固定幅→色付け）
    if ($LDAP_ENABLED){
        ...
        $etype_disp = format_etype($grp, $levelId, 10);

        if ($DRY_RUN){
            if ($exists){
                printf("Up!  [%3d] [%02d-%03d] [%s] [%-20s] [DRY] 更新 [%s......] [%s]\n",
                    $idx+1,$cmp_id,$user_id,$etype_disp,$uid,substr($passwd,0,3),$displayName);
            } else {
                printf("Add! [%3d] [%02d-%03d] [%s] [%-20s] [DRY] 追加 [%s......] [%s]\n",
                    $idx+1,$cmp_id,$user_id,$etype_disp,$uid,substr($passwd,0,3),$displayName);
            }
        } else {
            if ($exists){
                ...
                if ($ok){
                    printf("Add! [%3d] [%02d-%03d] [%s] [%-20s] [CON] 追加 [%s......] [%s]\n",
                        $idx+1,$cmp_id,$user_id,$etype_disp,$uid,substr($passwd,0,3),$displayName);
                } else {
