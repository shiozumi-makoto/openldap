...
/**
 * ldap_id_pass_from_postgres_set.php (refactored, 2025-10-17, ldapi対応)
 *
...
$options = getopt('', [
    ...
    'ldapi',           // ★ ldapi 接続を要求（URI未指定なら既定ldapiにする）
    'uri::',           // ★ 接続URIを明示指定（--uri=ldap[s]://… も可）
    'log::',           // ログファイル（任意・未使用）
...
// LDAP 環境（ldap有効時のみ使う）? ★ URI優先ロジックに変更
...
foreach ($rows as $idx => $row) {
    ...
    $entrySamba = [];
    if (isset($domain_sid) && $domain_sid) {
    ...
    if (isset($domain_sid) && $domain_sid) $objectClass[] = "sambaSamAccount";
