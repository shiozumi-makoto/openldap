...
# ldapi を明示（EXTERNAL 認証なので BIND_* 不要）
export LDAPURI='ldapi://%2Fusr%2Flocal%2Fvar%2Frun%2Fldapi'
# 一部のサブツール/引数は LDAP_URL を参照するので合わせておく
export LDAP_URL="${LDAP_URL:-$LDAPURI}"
# 任意: ソケット存在チェック（保険）
[[ -S /usr/local/var/run/ldapi ]] || {
  echo "[ERROR] ldapi ソケットが見つかりません: /usr/local/var/run/ldapi"; exit 1;
}
...
export FORCE_COLOR="${FORCE_COLOR:-1}"   # 一部ツールが見る
unset NO_COLOR                            # NO_COLOR があると無色化する実装もある
...
# まず LDAPURI が既に与えられていて ldapi:// なら、それを最優先で使う
if [[ -n "${LDAPURI:-}" && "${LDAPURI}" == ldapi://* ]]; then
  ...
  if [[ "$HOST_SHORT" == "ovs-012" || "$HOST_FQDN" == "ovs-012.e-smile.local" ]]; then
    # ローカルに /var/run/ldapi があるならそれを使う（無ければ ldaps のまま）
...
# PHP へ渡す接続先は、LDAPURI(優先) -> LDAP_URL の順で決定（明示上書きも可）
...
# === (A) LDAPURI 初期化（未指定なら推測）＋ ヘルパ =========================
# 既に LDAPURI が指定済みなら触らない
if [[ -z "${LDAPURI:-}" ]]; then
  if [[ "$LDAP_URL" == ldapi://* ]]; then
    # LDAP_URL が ldapi の時はそれを流用
    export LDAPURI="$LDAP_URL"
  else
    # 未設定で /var/run/ldapi があれば ldapi を優先
    if [[ -S /var/run/ldapi ]]; then
...
ldap_auth_flags() {
  if is_ldapi; then
    ...
    echo "-H ${LDAPURI:-$LDAP_URL} -D ${dn} -w ${pw}"
  fi
}
...
# === (B) BIND_PW 必須チェックの条件化（ldapi の時は要求しない） ============
if ! is_ldapi; then
...
now_ms() {
  ...
  if [[ "$t" =~ ^[0-9]{9,}$ ]]; then echo "${t:0:13}"; return; fi
  t="$("$PHP_BIN" -r 'echo (int)(microtime(true)*1000);' 2>/dev/null || true)"
...
# ===== STEP計測 & エラートラップ =====
CURRENT_STEP="(init)"
declare -A STEP_TIME_START_MS STEP_TIME_TOTAL_MS
trap 'echo "[FATAL] Failed at: ${CURRENT_STEP}"; exit 1' ERR

step() {
  CURRENT_STEP="$1"
  section "$CURRENT_STEP"
  STEP_TIME_START_MS["$CURRENT_STEP"]="$(now_ms)"
}
step_done() {
  local name="$1"
  local end_ms start_ms elapsed_ms
  end_ms="$(now_ms)"
  start_ms="${STEP_TIME_START_MS[$name]}"
  elapsed_ms=$(( end_ms - start_ms ))
  STEP_TIME_TOTAL_MS["$name"]="$elapsed_ms"
  printf "[TIME] %s: %d ms\n" "$name" "$elapsed_ms"
}
...
run_env() {
  ...
  # 子プロセスにカラー強制・LDAP/ベースDN系を明示注入（未定義でも安全展開）
  CLICOLOR="${CLICOLOR:-1}" \
  ...
  BIND_DN="${BIND_DN-}" \
  BIND_PW="${BIND_PW-}" \
  LDAP_URL="${PHP_LDAP_URL-}" \
  LDAP_URI="${PHP_LDAP_URL-}" \
...
run_sh() {
  ...
  # 子プロセスにカラー強制・LDAP/ベースDN系・DRY_RUNを注入
  CLICOLOR="${CLICOLOR:-1}" \
  CLICOLOR_FORCE="${CLICOLOR_FORCE:-1}" \
  FORCE_COLOR="${FORCE_COLOR:-1}" \
  TERM="${TERM:-xterm-256color}" \
...
CMD5_OPTS=(
  --home-root="$HOME_ROOT"
  --trash="$TRASH_DIR"
  --log="${PRUNE_LOG:-/var/logs_share/prune_home_dirs.log}"
  --ldap-delete
  --ldap-url="$LDAP_URL"
...
# ===== サマリ（ミリ秒）=====
echo
echo "==================== SUMMARY: STEP EXECUTION TIME (ms) ===================="
# 並びを固定したい場合は、表示順で明示列挙
ordered_steps=(
  "STEP1: Import/Update users (homes + LDAP)"
  "STEP2: Add all users to 'users' group (memberUid)"
  "STEP3: Auto-assign groups by gidNumber (memberUid)"
  "STEP4: Samba net groupmap add (posixGroup -> NT Domain group)"
  "STEP5: Prune home directories (+ LDAP delete)"
  "LOG ROTATE: delete old logs (>7 days)"
)
total_ms=0
for s in "${ordered_steps[@]}"; do
  if [[ -n "${STEP_TIME_TOTAL_MS[$s]+x}" ]]; then
    printf "  %-65s : %6d ms\n" "$s" "${STEP_TIME_TOTAL_MS[$s]}"
    total_ms=$(( total_ms + STEP_TIME_TOTAL_MS[$s] ))
  fi
done
printf "  %-65s : %6d ms\n" "[TOTAL]" "$total_ms"
echo "=========================================================================="
