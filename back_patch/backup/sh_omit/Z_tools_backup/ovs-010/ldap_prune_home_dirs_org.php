...
} catch (Throwable $e) { abortx("SQL(valid) 失敗: ".$e->getMessage(), $LOGFILE); }
$invalid2HomeSet = [];
$invalid2Count = 0;
try {
...
} catch (Throwable $e) { abortx("SQL(invalid) 失敗: ".$e->getMessage(), $LOGFILE); }
...
foreach ($targets as $path) {
  if ($TRASH_DIR) {
    ...
    if (!@rename($path, $dst)) { $FS_MOVE_ERR++; log_line($LOGFILE, "[ERR ] MOVE 失敗: {$path}"); }
    else { $FS_MOVE_OK++; }
  } else {
