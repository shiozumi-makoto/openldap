...
// 第3引数に ""（空文字）を渡すと .pgpass が使われません。
// null なら libpq が .pgpass を参照できます。
$pdo = isset($pgPass)
...
# // DB接続
# $pdo = new PDO("pgsql:host=192.168.61.10;dbname=accounting", "esmile_user", "esmile_user0123", [
#     PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION
# ]);
...
-- WHERE ( {$target_column_all} ) and ( p.user_id = 1 ) 
...
//	if($cmp_id == 1 and $user_id == 1) continue;
...
*/
    ...
    $familyKana = $row['姓かな'];
   	$givenKana  = $row['名かな'];
   	$middleName = trim($row['ミドルネーム']);		// 同姓同名の重複を回避する。業務管理システムの個人名簿登録で、重複を回避している。
    $uid = kanaToRomaji($familyKana) . '-' . kanaToRomaji($givenKana) . $middleName;

	// --- uid 生成直後に追加 ---
	$uid = strtolower($uid);
	// 許可: 英小文字・数字・ハイフン、先頭/末尾ハイフン不可、連続ハイフン可
	...
	// UID を正規化・バリデーション
#	$uid = strtolower($uid);
#	$uid = preg_replace('/[^a-z0-9-]/', '', $uid); // 英数とハイフンのみ
#	$uid = trim($uid, '-');                        // 先頭/末尾のハイフン除去

	// 空や短すぎる/ハイフンだけ等を除外
	if ($uid === '' || $uid === '-' || strlen($uid) < 2) {
	    echo "Wrn! 無効な uid を検出したためスキップ（cmp_id={$cmp_id}, user_id={$user_id})\n";
	    continue;
	}
...
// ldapsearch -x -D "cn=admin,dc=e-smile,dc=ne,dc=jp" -w es0356525566 -b "ou=Users,dc=e-smile,dc=ne,dc=jp" "uid=shiozumi*" | awk '{ print }'
// ldapsearch -x -D "cn=admin,dc=e-smile,dc=ne,dc=jp" -w es0356525566 -b "ou=Users,dc=e-smile,dc=ne,dc=jp" "(gidNumber=2012)"
// ldapsearch -x -D "cn=admin,dc=e-smile,dc=ne,dc=jp" -w es0356525566 -b "ou=Users,dc=e-smile,dc=ne,dc=jp" "(gidNumber=2012)" uid cn gidNumber
...
ldapsearch -x -D "cn=admin,dc=e-smile,dc=ne,dc=jp" -w es0356525566 -b "ou=Users,dc=e-smile,dc=ne,dc=jp" "uid=shiozumi*" \
| awk '/:: /{ 
    cmd = "echo " $2 " | base64 -d"; 
    cmd | getline decoded; 
    close(cmd); 
