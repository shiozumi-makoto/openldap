...
# 注意: set -u 環境でも未定義参照で落ちないように「-」付き展開を使用
...
# ===== 認証情報の決定（優先度: BIND_PW > LDAP_ADMIN_PW > BIND_PW_FILE）=====
: "${BIND_PW_FILE:=/root/.secrets/ldap_admin.pw}"   # 任意: ファイルから読む場合
if [ -z "${BIND_PW-}" ]; then
  if [ -n "${LDAP_ADMIN_PW-}" ]; then
    ...
    echo "[ERROR] BIND_PW が未設定です（BIND_PW / LDAP_ADMIN_PW / BIND_PW_FILE を用意）" >&2
    exit 1
  fi
fi
...
# 新旧UI判定（--ldap-url オプションの有無）
has_new_ui() {
  "$PHP_BIN" "$1" --help 2>&1 | grep -q -- '--ldap-url' || return 1
}
...
if has_new_ui "$F1"; then
  echo "[INFO] PHP tool(F1): new interface detected"
  echo "[RUN] $PHP_BIN $F1 --home-root=$HOME_ROOT --skel=$SKEL_DIR --mode=$HOME_MODE --log=$ADD_LOG --ldap-enable --ldap-url=$LDAP_URL --ldap-base-dn=$LDAP_BASE_DN --bind-dn=$BIND_DN --bind-pass=$(mask_pw "$BIND_PW") --ldap-posix=$LDAP_POSIX --gid-default=$GID_DEFAULT --login-shell=$LOGIN_SHELL --confirm"
  CMD1=( "$PHP_BIN" "$F1"
    --home-root="$HOME_ROOT" --skel="$SKEL_DIR" --mode="$HOME_MODE"
    --log="$ADD_LOG"
    --ldap-enable --ldap-url="$LDAP_URL"
    --ldap-base-dn="$LDAP_BASE_DN"
    --bind-dn="$BIND_DN" --bind-pass="$BIND_PW"
    --ldap-posix="$LDAP_POSIX" --gid-default="$GID_DEFAULT"
    --login-shell="$LOGIN_SHELL"
  )
  [[ "${DRY_RUN:-false}" == "true" ]] || CMD1+=( --confirm )
  run "${CMD1[@]}"
else
  echo "[INFO] PHP tool(F1): legacy interface detected"
  run_legacy_with_env "$F1"
fi
...
mkdir -p "$TRASH_DIR"
if has_new_ui "$F5"; then
  echo "[INFO] PHP tool(F5): new interface detected"
  echo "[RUN] $PHP_BIN $F5 --home-root=$HOME_ROOT --trash=$TRASH_DIR --log=$PRUNE_LOG --ldap-delete --ldap-url=$LDAP_URL --ldap-base-dn=$LDAP_BASE_DN --bind-dn=$BIND_DN --bind-pass=$(mask_pw "$BIND_PW") $( [ -n "${AGE_DAYS}" ] && printf -- '--age-days=%s ' "$AGE_DAYS" ) --confirm"
  CMD5=( "$PHP_BIN" "$F5"
    --home-root="$HOME_ROOT"
    --trash="$TRASH_DIR"
    --log="$PRUNE_LOG"
  ...
  echo "[INFO] PHP tool(F5): legacy interface detected"
  # 旧UI想定では引数なし実行（スクリプトが env を読む実装）
  echo "[RUN] (env) BIND_DN=${BIND_DN} BIND_PW=$(mask_pw "$BIND_PW") $PHP_BIN $F5"
  BIND_DN="$BIND_DN" BIND_PW="$BIND_PW" \
  LDAP_URL="$LDAP_URL" LDAP_URI="$LDAP_URL" \
  LDAP_BASE_DN="$LDAP_BASE_DN" BASE_DN="${BASE_DN}" \
  PEOPLE_OU="ou=Users,${BASE_DN}" \
  HOME_ROOT="$HOME_ROOT" TRASH_DIR="$TRASH_DIR" PRUNE_LOG="$PRUNE_LOG" \
  AGE_DAYS="${AGE_DAYS-}" LDAP_DELETE="true" \
  "$PHP_BIN" "$F5"
fi
