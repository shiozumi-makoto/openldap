...
function ensure_posix_group($link, string $dn, string $cn, ?int $gid, bool $confirm): array {
    ...
    if ($gid !== null && $current !== null && $gid !== $current) {
        // 安全のため、既存 gid と異なる場合は差し替えを実行（要件に応じて方針変更可）
        if ($confirm) {
            $ok = @ldap_modify($link, $dn, ['gidNumber'=>(string)$gid]);
...
foreach ($TARGET_GROUPS as $cn) {
    ...
    [$created,$fixed,$err] = ensure_posix_group($link, $dn, $cn, $gid, $CONFIRM);
    if ($err !== null) {
        warn("{$cn}: {$err}");
        continue;
    }
