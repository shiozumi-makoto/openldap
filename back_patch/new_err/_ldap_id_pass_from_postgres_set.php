 ...
 * - --init: 初期化モード。Upsertは行わず、DBの samba_id を NULL、(--ldap時は) People OU 直下の該当 uid を削除
 * - 画面表示: 旧ログ形式（Up! 行のみ、職位は色付き・10桁固定）/ 冒頭と末尾にバナー
 *
    ...
    foreach ($rows as $r) {
        ...
        if (!empty($cfg['ldap']) && $ds) {
            ...
            if ($seiKana !== '' && $meiKana !== '') {
                ...
                if ($login !== '' && !in_array($login, $cands, true)) $cands[] = $login;
            }
