 ...
 * - 条件: p.level_id <> 0 AND (srv01..srv05 のどれかが 1) AND (p.user_id >= 100 OR p.user_id = 1)
 * - UID: 「姓かな」「名かな」必須 + 「ミドルネーム」任意
 *         kakasi(UTF-8) → 小文字 → 英数字以外除去 → uid="sei-mei" . mid（ミドルは直結）
 ...
 * - ログ: かな欠落や LDAP 失敗は tools/ldap_uid_errors.log へ
 * - 表示: 旧ログ形式（Up! 行のみ）、冒頭/末尾の概要・統計を表示
 *
...
$schema = [
    ...
    // 既定
    'ldap'       => ['cli'=>'ldap','type'=>'bool','default'=>false,'desc'=>'LDAP 更新を実施'],
...
//============================================================
// LDAP 接続補強（dn/uri出力・People OU・SIDなど）
//============================================================
if ($cfg['ldap'] || $cfg['home']) {
    if (!$baseDn) {
        $baseDn = Env::str('LDAP_BASE_DN', 'dc=e-smile,dc=ne,dc=jp');
...
// 追加：0 は 99 に読み替え（err-cls 99 として扱う）
if ($lvl === 0) { $lvl = 99; }
    ...
    $gidNumber = $gidDefault; // ※必要なら GroupDef::fromLevelId($lvl)['gid'] を使う運用に差し替え可
    $homeLink  = "/home/{$login}";
...
// カラーコードを除外して見た目の長さを計算
$plain   = preg_replace('/\x1B\[[0-9;]*[A-Za-z]/', '', $lblCol);
$padLen  = 10 + strlen($lblCol) - strlen($plain);
$lblDisp = str_pad($lblCol, $padLen); // ← 10桁固定＋カラー考慮

printf(
    "Up!  [%3d] [%02d-%03d] [%s] [%-20s] [CON] 更新 [%-8s] [%s]\n",
    $idx, $cmp, $uidn, $lblDisp, $login, mask_pwd3($pwd), $jpName
);
...
/*
    ...
    // Up! 行（旧フォーマット）
    printf(
        "Up!  [%3d] [%02d-%03d] [%-10s] [%-20s] [CON] 更新 [%-8s] [%s]\n",
        $idx, $cmp, $uidn, $lblCol, $login, mask_pwd3($pwd), $jpName
    );
*/
...
function detect_domain_sid(?\LDAP\Connection $ds, string $baseDn): ?string {
    if (!$ds) return null;
    // よくある場所: sambaDomainName=* のsambaDomainオブジェクト
...
/** 色付け（adm=赤, dir=青, mgr=マゼンタ, mgs=シアン, ent=緑, err=明赤） */
function colorize_level(string $label): string {
    // ANSIカラーが不要ならここでそのまま返す実装でもOK
    $map = [
        'adm-cls' => "\033[31m", // red
        'dir-cls' => "\033[34m", // blue
        'mgr-cls' => "\033[35m", // magenta
        'mgs-cls' => "\033[36m", // cyan
        'stf-cls' => "\033[36m", // cyan (近い色)
        'ent-cls' => "\033[32m", // green
        'err-cls' => "\033[91m", // bright red
    ];
    foreach ($map as $k=>$c) {
...
function log_ldap_error(string $f, int $cmp, int $uid, string $login, string $op, string $dn, \LDAP\Connection $ds): void {
    ...
    // 画面には旧ログ風の Up! ラインだけにしたいので、ここでのエコーは抑制
}
