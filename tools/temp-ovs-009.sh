...
echo
echo "${BASE_DIR}/temp-ovs-009.sh [実行shell]"
echo
...
COMMON_URI_FLAG=(--uri='ldaps://ovs-009.e-smile.local:636' --bind-dn='cn=Admin,dc=e-smile,dc=ne,dc=jp' --bind-pass='es0356525566' --base-dn='dc=e-smile,dc=ne,dc=jp')
...
php "${BASE_DIR}/ldap_id_pass_from_postgres_set.php" --ldaps --ldap "${CONFIRM_FLAG[@]}" "${COMMON_URI_FLAG[@]}"
...
php "${BASE_DIR}/ldap_level_groups_sync.php" \
  ...
  --init-group "${CONFIRM_FLAG[@]}" "${COMMON_URI_FLAG[@]}" --ldapi --description \
  || true
...
php "${BASE_DIR}/ldap_smb_groupmap_sync.php" \
     "${COMMON_URI_FLAG[@]}" "${CONFIRM_FLAG[@]}" --all --verbose
