...
# ==========================================================
# deploy_happy_head.sh
#   コピー済みの root.cron ファイルを各ホストへデプロイ
#
#   使用例:
#     ./deploy_happy_head.sh --host=ovs-009,ovs-010
#
# ==========================================================

SSH_USER="root"
SRC_DIR="."              # ファイルのあるローカルディレクトリ
REMOTE_PATH="/var/www/happy/htdocs/ver401/inc/header.h"

PERM="775"               # 推奨パーミッション（cron用）
...
Example:
  ./deploy_happy_head.sh --host=ovs-009,ovs-010,ovs-011
USAGE
  exit 1
}
...
for host in "${HOSTS[@]}"; do
  ...
  src_file="${SRC_DIR}/header.h.${suffix}"
