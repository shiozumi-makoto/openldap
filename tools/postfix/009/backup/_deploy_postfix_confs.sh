...
SRC_DIR="${SRC_DIR:-/usr/local/etc/openldap/tools/postfix/009}"
...
main(){
  ...
  # 1) ローカルにファイルがあるかチェック（.009 があれば優先、無ければ無印）
  for f in "${FILES[@]}"; do
    if [[ -f "${SRC_DIR}/${f}.009" ]]; then
      :
    elif [[ -f "${SRC_DIR}/${f}" ]]; then
      :
    else
      die "${SRC_DIR}/${f}.009 も ${SRC_DIR}/${f} も見つかりません"
    fi
  done
...
# 対象ファイルをバックアップ（virtual-regexp も含む）
...
EOS
  ...
  for f in "${FILES[@]}"; do
    src="${SRC_DIR}/${f}.009"
    [[ -f "$src" ]] || src="${SRC_DIR}/${f}"
    scp -p "$src" "${SSH_USER}@${HOST}:${REMOTE_DIR}/${f}.tmp"
  done
