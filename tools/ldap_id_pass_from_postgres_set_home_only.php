...
function ensure_home(
    ...
    bool $apply,
    callable $dbg,
    bool $withMaildir = true
): void {


	// ------------------------------------------------------------
	// HOME が既に存在する場合は、何もしない
	// ------------------------------------------------------------
	if (is_dir($real)) {
		$dbg("home exists: skip all: {$real}");
		return;
	}
    ...
    if (!is_dir($real)) {
        if ($apply) {
            if (!@mkdir($real, 0770, true)) {
                fwrite(STDERR, "[ERROR] mkdir失敗: {$real}\n");
            } else {
                echo "  [MKD] {$real}\n";
            }
        } else {
    ...
    if ($real === $link) {
        ...
        $dbg("same path: symlink skip: {$link}");
            ...
            if ($to !== $real) {
                ...
                if ($apply) {
                    ...
                    if (@symlink($real, $link)) {
                        ...
                        fwrite(STDERR, "[ERROR] symlink失敗: {$link}\n");
                    }
